...
def look_up_pcdb_1(could_be_suburb):
    ...
    escape_could_be_suburb = re.escape(could_be_suburb)
    results = dtconfig.DB.query("""
        select distinct locality, state, pcode
        from pcdb
        where upper(locality) ~* $escape_could_be_suburb
    """, vars = locals())
...
class CleanNonDPID:
    ...
    def check_address(self, params):
            ...
            if _results:
                if len(_results) <= 1:
                    ...
                    for _result in _results:
                        ...
                        if _ratio > 0.85 and _ratio > max_ratio:
                            max_ratio = _ratio
                            final_result = address_info
        ...
        if final_result:
            ...
            s = difflib.SequenceMatcher(None, postcode.upper(), new_postcode.upper())
            _ratio = s.ratio()
            if _ratio > 0.85:
                return final_result
        ...
        if results:
            ...
            if len(thn) > 1:
                ...
                _results = self.paflinkr.similar_run(re.sub('[()]','',thn))
                _results.pop(0)
                if _results:
                    if len(_results) == 1:
                        address_info = _results[-1].split('\t')
        ...
        if final_result:
            ...
            new_postcode = final_result[-2]
            new_street_name = final_result[0]
            s = difflib.SequenceMatcher(None, suburb.upper(), new_suburb.upper())
            print 'last step'
            print suburb.upper()
            print new_suburb.upper()
            print 'end last step'
            _ratio = s.ratio()
            if _ratio > 0.7:
                return final_result
    ...
    def step5(self):
        try:
            select_query = """
                select *
                from %s
                where (mm_note = '' or mm_note ~* 'check') and mm_preclean !~* 'invalid'
            """ % (self.clean_table)
            results = dtconfig.DB.query(select_query)
            ...
            for result in results:
                ...
                if not re.search(r'^\s*$', result[self.state], re.I):
                    print 'im here'
                    is_oseas, country_code = find_oseas(result[self.state])
                    #print is_oseas
                    #print country_code
                    
                elif not re.search(r'^\s*$', result[self.suburb], re.I):
                    is_oseas, country_code = find_oseas(result[self.suburb])
                    #print is_oseas
                    #print country_code
                #sys.exit("TEst")    
                ...
                if (re.search(r'^Aus(tralia)?$', country, re.I) or country == '') and not is_oseas:
                    ...
                    if final_result:
                        ...
                        if final_result:
                            ...
                            #update address
                            if thn.upper() != new_street_name.upper() and not re.search(r'^\s*$', new_street_name, re.I):
                                for each_address in self.address_list:
                                    dtconfig.DB.query("""
                                    update %s
